...
namespace MyTests
{
    ...
    public class Lab4DemoTests
    {
        ...
        [Initialize]
        public void Setup() => _manager = new PasswordManager();

        public static IEnumerable<object[]> SampleData()
        {
            yield return new object[] { "Google", "alice", "pass123" };
            yield return new object[] { "Facebook", "bob", "secure456" };
            yield return new object[] { "GitHub", "charlie", "github789" };
        }

        [Fact(Description = "Параметризованный тест с TestCaseSource", Category = "Demo", Author = "Student")]
        [TestCaseSource(nameof(SampleData))]
        public void ParameterizedWithSourceTest(string service, string user, string pass)
        {
            var account = _manager.AddAccount(service, user, pass);
            Assert.Equal(service, account.ServiceName);
            Assert.Equal(user, account.UserName);
        ...
        [Fact(Category = "Smoke", Author = "Student", Priority = 1)]
        public void TestWithExpressionTree()
        {
            int a = 5, b = 3;
            Assert.WithInfo(() => a + b == 10, "Сумма не равна 10");
        }

        [Fact(Category = "Demo", Priority = 2)]
