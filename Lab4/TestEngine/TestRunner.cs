...
namespace TestEngine
{
    ...
    public class TestRunner
    {
        ...
        public async Task<IReadOnlyList<TestResult>> RunAllAsync(Func<MethodInfo, bool>? filter = null)
        {
            ...
            foreach (var testClass in testClasses)
            {
                ...
                foreach (var method in testMethods)
                {
                    ...
                    var testCaseSourceAttr = method.GetCustomAttribute<TestCaseSourceAttribute>();
                    if (testCaseSourceAttr != null)
                    {
                        var sourceMethod = testClass.GetMethod(testCaseSourceAttr.SourceMethodName,
                            BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic);
                        if (sourceMethod != null && sourceMethod.ReturnType == typeof(IEnumerable<object[]>))
                        {
                            var data = (IEnumerable<object[]>)sourceMethod.Invoke(null, null);
                            foreach (var parameters in data)
                            {
                                testTasks.Add(() => RunTestMethodAsync(testInstance, method, initializeMethod, cleanupMethod, parameters));
                            }
                            continue;
                        }
                    }
        ...
        public async Task<IReadOnlyList<TestResult>> RunAllSequentialAsync(Func<MethodInfo, bool>? filter = null)
        {
            var testClasses = _testAssembly.GetTypes()
                .Where(t => t.GetCustomAttribute<TestSuiteAttribute>() != null);
                ...
                foreach (var method in testMethods)
                {
                    ...
                    if (testCaseSourceAttr != null)
                    {
                        ...
                        if (sourceMethod != null && sourceMethod.ReturnType == typeof(IEnumerable<object[]>))
                        {
                            ...
                            foreach (var parameters in data)
                            {
                                await RunTestMethodAsync(testInstance, method, initializeMethod, cleanupMethod, parameters);
                            }
                            continue;
                        }
                    }
