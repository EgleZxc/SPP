...
namespace TestEngine
{
    public static class Assert
    {
        ...
        {
            if (!condition)
                throw new AssertionException(message ?? "Ожидалась истина, но получена ложь.");
        }

        public static void False(bool condition, string? message = null)
        {
            if (condition)
                throw new AssertionException(message ?? "Ожидалась ложь, но получена истина.");
        }

        public static void Equal<T>(T expected, T actual, string? message = null)
        {
            if (!Equals(expected, actual))
                throw new AssertionException(message ?? $"Ожидалась: {expected}, Фактически: {actual}");
        }
